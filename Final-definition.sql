...
CREATE TABLE IF NOT EXISTS users (
	...
	PRIMARY KEY (user_id),
	CONSTRAINT first_last UNIQUE (fname,lname)
	-- CONSTRAINT fk_list_id FOREIGN KEY (fk_list_id) 
...
VALUES ('Sara','Smith','1970-01-02'),('David','Atkins','1979-11-18'),('Daniel', 'Jensen', '1985-03-02');
-- VALUES ('Sara','Smith','1970-01-02');

-- -- test list (just add user_id and date)
INSERT INTO list(updated, fk_user_id) 
VALUES (CURRENT_TIMESTAMP, (SELECT u.user_id FROM users u WHERE u.fname = 'Sara' and u.lname = 'Smith'));

INSERT INTO list(updated, fk_user_id) 
VALUES (CURRENT_TIMESTAMP, (SELECT u.user_id FROM users u WHERE u.fname = 'Daniel' and u.lname = 'Jensen'));

INSERT INTO list(updated, fk_user_id) 
VALUES (CURRENT_TIMESTAMP, (SELECT u.user_id FROM users u WHERE u.fname = 'David' and u.lname = 'Atkins'));

-- -- test product
INSERT INTO product(name, product_url, photo_url) 
VALUES ('Flowtron BK-15D Electronic Insect Killer, 1/2 Acre Coverage', 'http://www.amazon.com/Flowtron-BK-15D-Electronic-Insect-Coverage/dp/B00004R9VZ/ref=sr_1_1','http://ecx.images-amazon.com/images/I/715ZROmiT0L._SY550_.jpg');
INSERT INTO product(name, product_url, photo_url) 
VALUES ('Yimby Tumbler Composter, Color Black', 'http://www.amazon.com/Yimby-Tumbler-Composter-Color-Black/dp/B009378AG2/ref=sr_1_1', 'http://ecx.images-amazon.com/images/I/71rKqDcllzL._SL1000_.jpg');
INSERT INTO product(name, product_url, photo_url) 
VALUES ('Coleman Cable 02356-05 40-Foot 16/3 Vinyl Landscape Outdoor Extension Cord, Green', 'http://www.amazon.com/Coleman-Cable-02356-05-Landscape-Extension/dp/B00004SQET/ref=pd_sim_86_6', 'http://ecx.images-amazon.com/images/I/71J7X6HYGUL._SL1500_.jpg');
...
VALUES ((SELECT product_id FROM product p WHERE p.name = 'Flowtron BK-15D Electronic Insect Killer, 1/2 Acre Coverage'), 
	(SELECT list_id FROM list l 
		...
		WHERE user_id = (SELECT user_id FROM users WHERE fname = 'Sara' and lname = 'Smith'))
	);
...
VALUES ((SELECT product_id FROM product p WHERE p.name = 'Yimby Tumbler Composter, Color Black'), 
	(SELECT list_id FROM list l 
		INNER JOIN users u ON u.user_id = l.fk_user_id
		WHERE user_id = (SELECT user_id FROM users WHERE fname = 'Daniel' and lname = 'Jensen'))
	);
...
INSERT INTO manufacturer(name) -- should fail, duplicate
VALUES ('Flowtron');

INSERT INTO manufacturer(name) -- should succeed
VALUES ('Yimby');
...
-- -- INSERT INTO stores(store_url, store_name)
-- -- VALUES (NULL, 'Gibberish');

INSERT INTO product_store
VALUES ((SELECT product_id FROM product p WHERE p.name = 'Flowtron BK-15D Electronic Insect Killer, 1/2 Acre Coverage'), (SELECT store_id FROM stores s WHERE s.store_name = 'Amazon'), 35.57);

INSERT INTO product_store
VALUES ((SELECT product_id FROM product p WHERE p.name = 'Coleman Cable 02356-05 40-Foot 16/3 Vinyl Landscape Outdoor Extension Cord, Green'), (SELECT store_id FROM stores s WHERE s.store_name = 'Amazon'), 13.49);

INSERT INTO product_store
VALUES ((SELECT product_id FROM product p WHERE p.name = 'Yimby Tumbler Composter, Color Black'), (SELECT store_id FROM stores s WHERE s.store_name = 'Amazon'), 91.00);