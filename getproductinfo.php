<?php
	...
	$action = "SELECT DISTINCT p.name, p.photo_url, lp.bought, FLOOR(ps.price) AS dollars, FLOOR(((ps.price - FLOOR(ps.price)) * 100)) AS cents, m.name, m.country, s.store_name, s.store_url, ps.product_url FROM product p 
	...
	if(!$stmt->bind_result($pname, $pho_url, $bought, $dollars, $cents, $mname, $country, $sname, $st_url, $pr_url)){
		echo "Bind failed: "  . $mysqli->connect_errno . " " . $mysqli->connect_error;
	}

	$stmt->fetch();

	$data = array("pname"=>$pname, "pho_url"=>$pho_url, "bought"=>$bought, "dollars"=>$dollars, "cents"=>$cents,
